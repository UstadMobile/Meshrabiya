...
class DistributedStorageManager(
    ...
    // Client wrapper instance (exposes client API in its own file)
    val client: DistributedStorageClient = DistributedStorageClient(this)
    ...
    internal fun chunkFile(file: File, fileId: String, chunkSize: Int): List<MeshChunk> {
    ...
    internal fun selectBestStorageNodeInternal(responses: List<StorageNodeResponse>): StorageNodeResponse? = selectBestStorageNode(responses)
     ...
     */
    internal suspend fun storeFileInternal(
        path: String,
        data: ByteArray,
        ...
        // Delegate to client implementation which now contains the heavy logic.
        client.storeFileImpl(path, data, priority, replicationLevel, owner, recipients)
    }

    /**
     * Public delegator entry point preserved on the manager for compatibility.
     * Delegates to the client API (which in turn calls the internal implementation).
     */
    ...
    internal fun readChunkBytes(file: File, chunk: MeshChunk): ByteArray {
     ...
     */
    internal fun updateStorageStats() {
    ...
    // Internal accessors for client/server implementation files
    internal fun getStagedSyncManager() = stagedSyncManager
    internal fun getStorageQuotaManager() = storageQuotaManager
    internal fun getEncryptionManager() = encryptionManager
    internal fun getBetaLogger() = betaLogger
    internal fun getVirtualNode() = virtualNode
    internal fun getChunkReplicaTracker() = chunkReplicaTracker
    internal fun getStorageDataStore() = storageDataStore
    internal fun getConnectionPool() = connectionPool
    internal fun getCoreGossipBroadcastService() = coreGossipBroadcastService
    internal fun getScope() = scope

    internal fun putFileMetadata(fileId: String, metadata: FileMetadata) {
